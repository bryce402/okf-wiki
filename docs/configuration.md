...
| `OBSIDIAN_REQUIRED_TRUST_FIELDS` | Comma-separated effective required trust fields. Allowed values: `base_confidence`, `generated`, `lifecycle`, `lifecycle_changed`, `stale_after`, `status`, `timestamp`, `updated`, `verified`; unknown values fail closed | `base_confidence,lifecycle,generated` for lint; also `generated` for standalone trust commands |
